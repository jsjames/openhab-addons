...
/**
 * Implements the thread to read and parse the input stream. Once a packet can be identified, it locates the
...
public class PentairParser implements Runnable {
    public static final int MAXPENTAIRPACKET = 50;
    ...
    @Override
    public void run() {
        logger.trace("parser thread started");
        ByteBuffer buf = ByteBuffer.allocate(MAXPENTAIRPACKET + 10);
        int c, c2;
            ...
            try {
                ...
                switch (parserstate) {
                    ...
                    case CMD_PENTAIR:
                        ...
                        if (length > MAXPENTAIRPACKET) {
                            logger.trace("Received packet longer than {} bytes: {}", MAXPENTAIRPACKET, length);
                            break;
                        }
                        ...
                        if (checksumInPacket != checksumCalc) {
                            logger.trace("Checksum error: {}!={}-{}", checksumInPacket, checksumCalc,
                                    PentairPacket.toHexString(buf));
                            break;
                        }

                        PentairPacket p = new PentairPacket(buf.array(), buf.limit());
                    ...
                    case CMD_INTELLICHLOR: // 10 02 00 12 89 90 xx 10 03
                        ...
                        if (length == -1) {
                            logger.debug("IntelliChlor Packet unseen: command - {}", c & 0xFF);
                            break;
                        }
                        ...
                        if ((byte) c != (byte) 0x10 || (byte) c2 != (byte) 0x03) {
                            logger.trace("Invalid Intellichlor command: {}", PentairPacket.toHexString(buf));
                        ...
                        if ((byte) checksumCalc != (byte) checksumInPacket) {
                            logger.trace("Invalid Intellichlor checksum: {}", PentairPacket.toHexString(buf));
                            break;
                        }
